...
#region debug-only
        ...
        private void CheckSurfaceSampleTransmit()
        {
            ...
            if (exp.BiomeIsRelevantWhile(ScienceUtil.GetExperimentSituation(FlightGlobals.ActiveVessel)))
            {
                ...
                Log.Debug("sample count: " + API.ScienceAlert.VesselDataCache.FindStoredData(subject.id).Count);
                ...
                Log.Normal("Next recovered sample worth: {0}", subject.CalculateNextReport(exp, API.ScienceAlert.VesselDataCache.FindStoredData(subject.id)));
                ...
                Log.Normal("next transmitted sample worth: {0}", subject.CalculateNextReport(exp, API.ScienceAlert.VesselDataCache.FindStoredData(subject.id), xmitScalar));
            }
        }
